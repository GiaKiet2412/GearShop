...
router.post('/rate-order', async (req, res) => {
    ...
    try {
        ...
        const threeMonths = 3 * 30 * 24 * 60 * 60 * 1000; // 3 tháng tính bằng milliseconds

        if (currentDate - orderDate > threeMonths) {
            return res.status(400).json({ message: 'Thời gian đánh giá đã hết hạn (3 tháng).' });
        }
